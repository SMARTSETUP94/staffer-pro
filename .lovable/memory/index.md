...
52. ✅ **v0.44.3 Sprint correctif** (10 mai 2026, ~7h) — Top 3 audit livrés : (a) `ScopedAccessBanner` chef_metier_scoped sur `/affaires`, `/validation-heures`, `/mobile/chef/dashboard` + E2E stub ; (b) Triggers business `validate_heures_saisies_bounds` (heures_reelles ∈[0,24], heures_nuit ∈[0,24], nuit≤reelles) + `validate_contrat_intermittent` (date_fin≥date_debut, taux>0) + `validate_assignation_heures` (∈[0,24]) avec codes métier `HEURES_INVALIDES`/`DATES_CONTRAT_INVALIDES`/`TAUX_INVALIDE` ; (c) Soft-delete audit trail : `deleted_by` sur `affaire_documents` + `deleted_at`/`deleted_by` sur `fabrication_objets_photos` + RPC `soft_delete_affaire_document`/`soft_delete_objet_photo` + vue admin `v_documents_supprimes_30j` + RLS filter `deleted_at IS NULL` ; (d) Page `/admin/audit` 3 onglets (validations heures, docs supprimés 30j, uploads récents) + export CSV ; (e) pgTAP `supabase/tests/heures_saisies_bounds.sql` 8/8 verts (bornes hors-limites + bornes incluses + trigger assignations). Voir `docs/sprint-v0443-checklist.md`.
53. ✅ **v0.44.4 Polish audit** (10 mai 2026, ~4h) — Top 5 audit restant livré : (a) `useAffaireDocuments.prefetchSignedUrls` via `createSignedUrls` bulk (20 photos = 1 round-trip au lieu de 20) + galerie déclenche le préfetch sur `documents` change ; (b) `DocumentThumbnail` lazy via `IntersectionObserver` (rootMargin 200px) — `getSignedUrl` appelé seulement à l'entrée viewport, en général cache-hit grâce au préfetch ; (c) `src/lib/business-errors.ts` (`parseBusinessError` + `formatBusinessError`) mappe codes triggers `HEURES_INVALIDES`/`DATES_CONTRAT_INVALIDES`/`TAUX_INVALIDE`/`VOLUME_ECART_DEVIS` en messages FR + 6/6 tests vitest ; (d) 3 ADRs livrés (`docs/adr/001-rls-scoped-chef-metier.md`, `002-objet-id-nullable-affaire-documents.md`, `003-tiptap-pour-contrats-cddu.md`) + index README ; (e) Seed E2E `chef_metier_scoped` : nouveau `TestRole` + 4e compte dans `e2e/seed.ts` avec affaire dédiée `5E2E2` + `ensureChefAffaire` paramétré + storage state `chef-scoped.json`. Index DB déjà présents (`idx_affaire_documents_affaire_active` + `idx_fab_photos_active`) — audit avait sous-estimé l'existant. `/mobile/chef/a-valider` conservé (redirect intentionnel v0.44.2, pas dead code). Voir `docs/sprint-v0444-checklist.md`.
54. ✅ **v0.44.5 Durcissement audit** (10 mai 2026, ~1h) — Bloc 🟠 audit livré : (a) Migration RLS `affaire_documents_select` masque les soft-deleted (`deleted_at IS NULL OR is_admin()`) — aligné avec `fabrication_objets_photos` ; (b) Trigger `enforce_signed_at_server_side` BEFORE INSERT sur `contrats_signatures` force `signed_at = now()` + `created_at = now()` (anti-manipulation client malveillant) ; (c) `formatBusinessError` branché dans `ValiderHeuresList` (3 sites validate/correct/reject), `SaisirPourEmployeDialog` (catch), `BulkSaisieDialog` (preview error + catch bulk) — toasts utilisateur en FR pour `HEURES_INVALIDES`/`DATES_CONTRAT_INVALIDES`/`TAUX_INVALIDE`/`VOLUME_ECART_DEVIS` au lieu de `new row violates RLS…`.
55. ⏸️ **v0.45 RLS hardening (suite)** — Réouvrable post-v0.44.4 : pgTAP CI sur `mes_affaires_chef` + politiques RLS DB scopées sur heures/assignations/docs/photos + E2E isolement chef scopé.
