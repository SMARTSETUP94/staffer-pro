...
53. ⏳ **v0.44.4 Polish audit** (~4h) — Top 5 audit restant : batch signed URLs galerie + index `uploaded_at DESC` + lazy-loading + 3 ADR (RLS scoped, objet_id, TipTap) + purge dead code `/a-valider` + seed `chef_metier_scoped` dédié + toast UI mappant codes métier triggers.
54. ⏸️ **v0.45 RLS hardening (suite)** — Réouvert post-v0.44.4 (pgTAP CI + RLS scopée durcie côté DB).
