...
Heures de fabrication (LOT 3) : SOURCE DE VÉRITÉ = table `objet_heures_metier` (objet × métier, origine devis/ajout, note, sous-traitance). Les 7 colonnes `fabrication_objets.heures_prevues_*` sont un CACHE maintenu par le trigger `sync_objet_heures_colonnes()` — ne jamais les écrire directement ni les supprimer (algo staffing + MV `v_objet_heures_consolidees`). Lots `fabrication_lots` : les heures vivent TOUJOURS sur l'objet, JAMAIS sur le lot. Effectif prévisionnel anonyme = `atelier_planning` ; nommage via `assignations.atelier_planning_id`. Voir mem://features/lot3-modele-fabrication.
...
- [Assignations — source unique](mem://constraints/assignations-source-unique) — helper `src/lib/assignation-upsert.ts` + garde-fou Vitest. 7 surfaces staffing unifiées (AssignationDialog, Bulk×3, PlanningGrid, CellEditDialog, use-bulk-assign-objet). `created_by` posé auto sur chaque création.
- [LOT 1 — Vue Échéances](mem://features/lot1-vue-echeances) — page `/echeances` (cap `section.affaires`, sidebar Pilotage) : opérations Montage/Démontage groupées par semaine, fenêtre 14j/1m/3m persistée URL, filtres type/statut/recherche, compteurs (dont nuits, prospects, affaires sans date de montage). Champs `affaires.montage_*` (nb_techniciens, travail_nuit, nb_semi, nb_20m3, nature_prestation, notes) éditables dans « Dates clés chantier ».
- [LOT 2 — Liste affaires orientée échéance](mem://features/lot2-affaires-echeance) — `/affaires` : colonnes Montage (J−n) / Démontage / Chef de projet / Chargé d'affaires, « Période » retirée, tri par en-tête persisté URL (`?tri=&sens=`, défaut `date_montage` asc nulls last), filtre « Montage dans… » (14j/1m/3m/Sans date) et compteur « X affaires · Y sans date de montage » cliquable.

- [LOT 5 — Planning atelier deux temps](mem://features/planning-atelier-deux-temps) — `atelier_planning` = effectif ANONYME (nb_pers/métier/jour/objet ou lot, 8h/jour) ; nommage nominatif POSTÉRIEUR et optionnel dans `assignations` (via `assignation-upsert.ts` uniquement, `atelier_planning_id` + alimentation systématique de `assignation_objets`). Onglet « Planning » fiche affaire.
- [LOT 6 — Charge atelier `/charge`](mem://features/charge-atelier-atelier-planning) — matrice métiers × jours alimentée par `atelier_planning` (PAS les plans staffing publiés — `/charge-atelier` legacy reste vide en prod). Seuils de couleur relatifs à `metiers.capacite_jour` (vert < / ambre = / rouge >), neutre si capacité NULL. Filtres métier/chantier/prospects/sous-traitance persistés URL + drill-down cellule.
- [LOT B — Tableau d'atelier](mem://features/lot-b-tableau-atelier) — `/atelier` 5 colonnes (BE/NUM/FAB/FIN/MAN), règle de complétude SQL `etape_prete` = SOURCE UNIQUE (jamais dupliquée en TS), RPC `valider_etape`/`invalider_etape` non bloquantes, statut `en_attente_validation`, trigger cohérence heures↔étapes, `/charge-atelier` supprimée (redirect `/charge`).
- [Lot B-bis/D/E atelier](mem://features/lot-b-bis-d-e) — validation étape en 2 temps, CNC en avertissement soft, source inbox `atelier_rappel`, bloc Mon atelier scopé 5XXX/6XXX/9XXX
- [LOT 7 — Qui est où](mem://features/planning-qui-est-ou) — `/qui-est-ou` grille personnes × jours depuis `assignations`, vue inversée par chantier, anomalies (double affectation / affecté-absent), KPIs occupation absences déduites, écart effectif prévu (`atelier_planning`) vs nommé.
- [LOT 8 — Import planning Excel](mem://features/import-planning-excel) — `/imports/planning` (cap `section.admin`, onglet ImportsTabsNav) : parseur pur `src/lib/imports/planning-xlsx.ts` (onglets Fabrication/Livraisons/Affectations, en-têtes souples), plan pur `planning-plan.ts`, écritures idempotentes `planning-apply.ts`. **1 personne-jour = 8 h** (`HEURES_PAR_PERSONNE_JOUR`), `origine='ajout'` par défaut. Mapping métiers extensible (Bois→1, Métal→2, Peinture→3, Numérique→4, Tapisserie→5, Machiniste→6, Logistique→7, BE→8, Impression UV→9) ; `Sous-traitance` = marqueur, pas un métier. Idempotence : objets `(affaire,nom)`, heures upsert `(objet,métier)` remplacer/cumuler, planning `(affaire,objet,métier,date)`, assignations `(employé,affaire,date)` via `assignation-upsert.ts`. Aucune affaire ni employé créés en silence.
- [Déploiement CA + planning général](mem://features/deploiement-charges-affaires-planning-general) — flag menu réduit + /planning-general frise par métier ; règle validateSearch toujours optionnel
