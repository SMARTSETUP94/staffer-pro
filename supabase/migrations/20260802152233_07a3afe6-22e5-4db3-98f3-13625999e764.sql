-- B-bis : terminer_etape (déclaration de fin par l'opérateur)
CREATE OR REPLACE FUNCTION public.terminer_etape(_etape_id uuid, _commentaire text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT id INTO emp_id FROM public.employes WHERE user_id = uid;

  peut := public.is_admin()
       OR (o.respo_fab_id IS NOT NULL AND o.respo_fab_id = uid)
       OR (e.assignee_id IS NOT NULL AND e.assignee_id = uid)
       OR (emp_id IS NOT NULL AND EXISTS (
             SELECT 1 FROM public.fabrication_objet_equipe foe
             WHERE foe.objet_id = e.objet_id AND foe.employe_id = emp_id
           ));

  IF NOT peut THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Seule une personne de l''équipe de l''objet peut déclarer l''étape terminée');
  END IF;

  -- Sur l'étape du responsable fabrication, terminer vaut validation.
  IF e.type_etape = 'respo_fab' THEN
    RETURN public.valider_etape(_etape_id, _commentaire);
  END IF;
  ...
  VALUES (
    ...
    e.id,
    jsonb_build_object(
      'type_etape', e.type_etape::text,
      'statut', 'en_attente_validation',
      'commentaire', NULLIF(TRIM(COALESCE(_commentaire, '')), '')
    )
  );

  RETURN jsonb_build_object('ok', true, 'etape_id', _etape_id, 'statut', 'en_attente_validation');
END;
$function$;

REVOKE ALL ON FUNCTION public.terminer_etape(uuid, text) FROM public;
GRANT EXECUTE ON FUNCTION public.terminer_etape(uuid, text) TO authenticated;

-- D2 : capability du rappel atelier
INSERT INTO public.capabilities (key, label, category, sort_order)
VALUES ('inbox.atelier_rappel', 'Rappels atelier (plan non publié)', 'inbox', 12)
ON CONFLICT (key) DO NOTHING;

INSERT INTO public.role_capabilities (role, capability, granted, scope)
VALUES ('admin', 'inbox.atelier_rappel', true, 'all'),
       ('bureau_etude', 'inbox.atelier_rappel', true, 'all')
ON CONFLICT (role, capability) DO UPDATE SET granted = true, scope = EXCLUDED.scope;

-- D2 : source atelier_rappel dans get_inbox_items
...
DECLARE
  ...
  v_cap_atelier_rappel     boolean := public.user_has_cap('inbox.atelier_rappel');
BEGIN
  ...
  atelier_rappel AS (
    SELECT
      'atelier_rappel:' || fo.id::text,
      'atelier_rappel'::text,
      fo.id,
      CASE
        WHEN d.echeance IS NOT NULL AND d.echeance <= CURRENT_DATE + INTERVAL '7 days' THEN 'high'
        WHEN d.echeance IS NOT NULL THEN 'medium'
        ELSE 'low'
      END::text,
      ('Plan technique non publié — ' || COALESCE(fo.nom, fo.reference, 'objet')),
      (COALESCE(af.numero,'?') || ' · ref ' || COALESCE(fo.reference,'?') ||
        COALESCE(' · échéance ' || to_char(d.echeance,'DD/MM'), ' · sans échéance')),
      fo.affaire_id, af.numero,
      ('/affaires/' || fo.affaire_id::text || '/objets/' || fo.id::text),
      COALESCE(d.echeance::timestamptz, fo.created_at)
    FROM public.fabrication_objets fo
    LEFT JOIN public.affaires af ON af.id = fo.affaire_id
    LEFT JOIN LATERAL (
      SELECT COALESCE(
        (SELECT MIN(ap.date) FROM public.atelier_planning ap
          WHERE ap.objet_id = fo.id AND ap.date >= CURRENT_DATE),
        af.date_montage
      ) AS echeance
    ) d ON true
    WHERE v_cap_atelier_rappel
      AND fo.archive = false
      AND COALESCE(af.statut::text, '') NOT IN ('termine','annule')
      AND fo.plan_url IS NULL
      AND NOT EXISTS (
        SELECT 1 FROM public.affaire_documents ad
        WHERE ad.objet_id = fo.id
          AND lower(COALESCE(ad.nom_fichier, '')) LIKE '%.pdf'
      )
      AND EXISTS (
        SELECT 1 FROM public.fabrication_etapes fe
        WHERE fe.objet_id = fo.id
          AND fe.type_etape IN ('usinage','respo_fab')
          AND fe.statut NOT IN ('non_applicable','termine')
      )
  ),
  devis_brouillon AS (
    SELECT
  ...
  SELECT * FROM (
    ...
    UNION ALL SELECT * FROM atelier_rappel
