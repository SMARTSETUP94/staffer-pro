-- Casse la récursion entre affaires <-> affaire_equipe
-- La policy FOR ALL ajoutait une qual au SELECT qui interrogeait affaires
-- => on la scinde en INSERT/UPDATE/DELETE uniquement
DROP POLICY IF EXISTS affaire_equipe_modify_chef_admin ON public.affaire_equipe;

CREATE POLICY affaire_equipe_modify_chef_admin_ins
...
CREATE POLICY affaire_equipe_modify_chef_admin_upd
  ON public.affaire_equipe
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY affaire_equipe_modify_chef_admin_del
  ON public.affaire_equipe
  FOR DELETE
  TO authenticated
  USING (
