-- L4a-bis HOTFIX — la CTE mission_pose pointait sur `assignations.phase` (colonne
-- inexistante), ce qui faisait échouer toute la RPC get_inbox_items pour tous
-- les rôles (même sans cap inbox.mission_pose). Bonne table : affaire_equipe
-- (Sprint A) avec colonne phase CHECK ('commercial_etude','fabrication',
-- 'logistique','montage','demontage'). Dates lues sur affaires.date_montage /
-- date_demontage.
--
-- Patch ciblé : on remplace UNIQUEMENT la CTE mission_pose ; les 5 autres
-- nouvelles sources L4a-bis (validation_heures, be_attente, devis_brouillon,
-- heures_saisir, rh_contrats) et toutes les sources legacy sont inchangées.
...
DECLARE
  ...
  v_flag_active      boolean := public.is_feature_active('equipes_3_niveaux_alertes');
...
BEGIN
  ...
  -- === SOURCE: mission_pose === (L4a-bis HOTFIX — sur affaire_equipe)
  mission_pose AS (
    SELECT
